...
exports.askedin=
    passport.authenticate('jwt', { session: false }),
    async (req, res) => {
        let { questions } = await Course.findByPk(req.params.cid).populate(
            'questions',
            null,
            null,
            { sort: { sno: 1 } },
        ...
        for (i = 0; i < questions.length; i++) {
            const user = await User.findByPk(questions[i].uid).select(
...
exports.del=
    passport.authenticate('jwt', { session: false }),
    async (req, res) => {
        let question = await Question.findByPk(req.params.qid).populate('answers');
